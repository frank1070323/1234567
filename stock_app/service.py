    ...
    def analyze(
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            profile_future = executor.submit(
                self.supplemental_service.fetch_company_profile,
                symbol,
                history.market,
                history.name,
            )
            revenue_future = executor.submit(
                self.supplemental_service.fetch_revenue_metrics,
                symbol,
                history.market,
            )
            valuation_future = executor.submit(
                self.supplemental_service.fetch_valuation_metrics,
                symbol,
                history.market,
            )
            institutional_future = executor.submit(
                self.supplemental_service.fetch_institutional_metrics,
                symbol,
                history.market,
            )
